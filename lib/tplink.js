...
// App-global signing key pairs, extracted once from the official Tapo Android app
// (NOT per-user secrets): the auth pair is hardcoded in the app's code; the passthrough
// pair ships in the app's assets/sdkconfig.xml. Same for every install, so we bake them in
// and end-users only supply email + password.
const APP_KEYS = {
  auth:        { accessKey: '4d11b6b9d5ea4d19a829adbb9714b057', secret: '6ed7d97f3e73467f8a5bab90b577ba4c' },
  passthrough: { accessKey: 'e37525375f8845999bcc56d5e6faa76d', secret: '314bc6700b3140ca80bc655e527cb062' },
};
...
function commonQS(cfg, extra = '') {
  const p = new URLSearchParams({
    ...
    termID: cfg.terminalUUID, ospf: 'Android 11', brand: 'TPLINK', locale: 'en_US',
    model: 'Homebridge', termName: 'Homebridge', termMeta: '1',
  });
...
class TapoCloud {
  constructor(cfg, log) {
    ...
    this.log = log;
    this.token = null;
    this.auth = cfg.auth || APP_KEYS.auth;          // app-global constant
    this.pt = cfg.passthrough || APP_KEYS.passthrough; // app-global constant
  ...
  async login() {
    const { email, password, terminalUUID } = this.cfg;
    const r = await signedPost(this.loginHost, '/api/v2/account/login', commonQS(this.cfg), {
      ...
      terminalMeta: '1', terminalName: 'Homebridge', terminalUUID,
    }, this.auth.accessKey, this.auth.secret);
  ...
  async passthrough(deviceId, requestData, retry = true) {
    ...
    const r = await signedPost(this.appServerHost, '/api/v2/common/passthrough',
      ...
      this.pt.accessKey, this.pt.secret);
  ...
  async setCameraEnabled(deviceId, enable) {
    return this.setFeature(deviceId, 'smartlife.cam.ipcamera.switch', 'set_is_enable', enable);
  }
  ...
  // Generic on/off feature: { <namespace>: { <method>: { value: "on"|"off" } } } -> err_code 0
  async setFeature(deviceId, ns, setMethod, on) {
    const resp = await this.passthrough(deviceId, { [ns]: { [setMethod]: { value: on ? 'on' : 'off' } } });
    const err = resp[ns][setMethod].err_code;
    if (err !== 0) throw new Error(`${ns}.${setMethod} rejected: err_code ${err}`);
  }

  async getFeatureCloud(deviceId, ns, getMethod) {
    const resp = await this.passthrough(deviceId, { [ns]: { [getMethod]: {} } });
    return resp[ns][getMethod].value === 'on';
  }
}
...
module.exports = { TapoCloud, getCameraEnabledLocal, getLocalSysinfo, APP_KEYS };
